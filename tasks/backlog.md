# Mamalik v0.1 Backlog

Canonical Markdown backlog for active v0.1 work.

## Sprint 1 - Foundation + Kingdom Creation

- [x] S1-001: Initialize repository foundation and persistent memory files.
- [ ] S1-002: Initialize minimal project structure and tooling.
- [ ] S1-003: Configure Next.js, TypeScript, Tailwind, lint, and basic scripts.
- [ ] S1-004: Configure Prisma and PostgreSQL/PostGIS foundation.
- [ ] S1-005: Create initial v0.1 Prisma models.
- [ ] S1-006: Implement email/password auth.
- [ ] S1-007: Implement Google login.
- [ ] S1-008: Add protected route behavior for no-kingdom users.
- [ ] S1-009: Create MapLibre kingdom creation page.
- [ ] S1-010: Add click marker and coordinate selection.
- [ ] S1-011: Create temporary validate-location API.
- [ ] S1-012: Create kingdom creation API.
- [ ] S1-013: Seed starter districts, resources, buildings, and units.
- [ ] S1-014: Create basic kingdom dashboard.
- [ ] S1-015: Create basic admin views.

## Sprint 2 - Tick Engine + Economy

- [ ] See `tasks/sprint_02.md`.

## Sprint 3 - Land Buying + District Management

- [ ] See `tasks/sprint_03.md`.

## Sprint 4 - Map Validation + Borders

- [ ] See `tasks/sprint_04.md`.

## Sprint 5 - Movement + Scouting + Combat

- [ ] See `tasks/sprint_05.md`.

## Sprint 6 - Alliances + Reports + Rankings + Admin Polish

- [ ] See `tasks/sprint_06.md`.
